...
# Function to get the account name for a given AWS account ID
def get_account_name(account_id):
    ...
    try:
        ...
        print(f"Error fetching account name for {account_id}: {str(e)}")
...
end_date = datetime.datetime(2023, 9, 7)    # Replace with your desired end date
...
for account_id in account_ids:
    ...
    for data_point in daily_costs:
        ...
        daily_cost_data.append({'Account ID': account_id, 'Account Name': account_name, 'Date': date, 'Cost': cost})
...
with open(output_file, 'w', newline='') as csvfile:
    fieldnames = ['Account ID', 'Account Name', 'Date', 'Cost']
